...
export class AddDonorComponent implements OnInit {
  ...
  submitForm() {
    ...
    const { bloodBank, ...formValueWithoutBloodBank } = this.donorForm.value;
    formValueWithoutBloodBank.transfusion_date = formattedDate;
    formValueWithoutBloodBank.bank = {
    ...
    formValueWithoutBloodBank.location = this.bloodbanks[0].location;
    formValueWithoutBloodBank.eligibility = 'yes';

    console.log(formValueWithoutBloodBank); // Check if the form data is correctly prepared without the bloodBank property

    this.dataRepo.bloodDonation(formValueWithoutBloodBank); // Send the form data to the backend
  }
...
get bloodbanks()
{
  ...
  minDate: Date = new Date();
    maxDate: Date = new Date(this.minDate.getFullYear(), this.minDate.getMonth() , this.minDate.getDate()+1)
  // setMinMaxDate() {
  //   this.today = new Date();
